...
public class Stage
{
    ...
    {
        var actor = FindActorInCast(actorName) ?? Cast.WithActorNamed(actorName);
        ActorInTheSpotlight = actor;
    }

    private Actor FindActorInCast(string actorName)
    {
        return Cast.Actors.FirstOrDefault(a => a.Name.Equals(actorName, StringComparison.OrdinalIgnoreCase));
    }
}